...
## 15. 要修正・未実装タスク
- [ ] CSP設定の実装
  - [ ] next.config.jsでのContent-Security-Policy設定（仕様書4.8.1参照）
  - [ ] その他のセキュリティヘッダー設定（X-Content-Type-Options, X-Frame-Options など）
- [ ] 統計データの実際のデータ使用
  - [ ] DistributionPieChartでのモックデータをAPIデータに置き換え（優先度・タグ分布）
- [ ] タグと優先度のフィルタリング実装強化
  - [ ] DataGridでのタグでのフィルタリング機能の完全実装
  - [ ] 優先度フィルターの完全実装
- [ ] レスポンシブデザイン改善
  - [ ] モバイル画面でのダッシュボードレイアウト調整
  - [ ] タスク一覧画面のモバイル対応強化
- [ ] エラー監視設定
  - [ ] Sentryなどの外部エラー監視サービスの導入検討
- [ ] 国際化対応
  - [ ] プレースホルダだけでなくi18nライブラリを使用した国際化対応 